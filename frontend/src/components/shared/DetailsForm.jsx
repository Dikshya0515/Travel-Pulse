...
import { useUpdateMeMutation, useUpdateUserByIdMutation } from '../../redux/apis/userApi';
...
export default function DetailsForm({ name, email, userPhone, photo, userId }) {
  ...
  const [updateMe, { isLoading: updateMeLoading, data: updateMeData, error: updateMeError }] =
    useUpdateMeMutation();
  const [
    updateById,
    { isLoading: updateByIdLoading, data: updateByIdData, error: updateByIdError },
  ...
  const submitHandler = (e) => {
    ...
    // Only append phone if it has a valid value
