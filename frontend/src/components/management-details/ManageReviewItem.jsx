...
export default function ManageReviewItem({ reviewObj }) {
  ...
  const [deleteReview, { isLoading, error, isSuccess }] = useDeleteReviewMutation();
  const { user, tour, review } = reviewObj;

  useEffect(() => {
    if (error) dispatch(setAlert({ type: 'error', msg: error }));

    if (isSuccess) dispatch(setAlert({ type: 'success', msg: 'Review was deleted.' }));
    ...
    <Fragment>
      ...
      <div className="reviews-table-grid">
        <div className="table-img-item">
          <img className="manage-user-img" src={user.photo ? getUserImageUrl(user.photo) : '/img/users/default.jpg'} alt={user.name} />
          <div>
            <h3>{user.name}</h3>
        ...
        <Actions onEdit={() => setReviewModal(true)} onDelete={() => setModal(true)} />
      </div>
      ...
      <p className="manage-review-text">
        {review.length > 200 ? (
          <Fragment>
            ...
            <span className="read-more" onClick={() => setReadMore((prevState) => !prevState)}>
